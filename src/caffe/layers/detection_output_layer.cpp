...
void DetectionOutputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  // from caffe.Net('deploy.prototxt', caffe.TEST),
  // bottom[0] = 1, 51000
  // bottom[1] = 1, 146625
  // bottom[2] = 1, 2, 25500
  CHECK_EQ(num_priors_ * num_loc_classes_ * 8, bottom[0]->channels())
      << "Number of priors must match number of location predictions.";
  CHECK_EQ(num_priors_ * (num_classes_ + 2), bottom[1]->channels())
...
void DetectionOutputLayer<Dtype>::Forward_cpu(
  ...
  const Dtype* odm_arm_loc_data = bottom[0]->cpu_data(); // 1 51000
  const Dtype* odm_arm_conf_data = bottom[1]->cpu_data(); // 1 146625
  const Dtype* prior_data = bottom[2]->cpu_data(); // 1 2 25500
  const Dtype* arm_conf_data = NULL;
  const Dtype* arm_loc_data = NULL;
  const int num = bottom[0]->num(); // 1
  ...
  //}
  const Dtype *loc_data = odm_arm_loc_data; // 25500
  const Dtype *conf_data = odm_arm_conf_data; // 133875
  arm_loc_data = &odm_arm_loc_data[25500]; // 25500
  arm_conf_data = &odm_arm_conf_data[133875]; // 12750
