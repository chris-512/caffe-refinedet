...
def CreateRefineDetHeadForTest(net, data_layer="data", num_classes=[], from_layers=[], from_layers2=[],
        normalizations=[], use_batchnorm=True, lr_mult=1, min_sizes=[], max_sizes=[], prior_variance = [0.1],
        aspect_ratios=[], steps=[], img_height=0, img_width=0, share_location=True,
        flip=True, clip=True, offset=0.5, inter_layer_depth=[], kernel_size=1, pad=0,
        conf_postfix='', loc_postfix='', **bn_param):
    ...
    mbox_layers = []
    odm_arm_loc = [net['odm_loc'], net['arm_loc']]
    net['odm_arm_loc'] = L.Concat(*odm_arm_loc, axis=1)
    mbox_layers.append(net['odm_arm_loc'])
    odm_arm_conf = [net['odm_conf'], net['arm_conf']]
    net['odm_arm_conf'] = L.Concat(*odm_arm_conf, axis=1)
    mbox_layers.append(net['odm_arm_conf'])
    mbox_layers.append(net['arm_priorbox'])
